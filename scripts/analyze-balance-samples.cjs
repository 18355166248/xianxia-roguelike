...
function formatMarkdown(reports, sourceNames, sampleCount) {
  const lines = [
    '# P3 数值样本汇总',
    '',
    `来源：${sourceNames.join('、')}；有效对局：${sampleCount}。`,
    '',
...
function main(args) {
  ...
  try {
    ...
    console.log(formatMarkdown(reports, paths.map(basename), samples.length));
