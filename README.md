...
- WASD / 方向键移动；移动端点击战场任意位置，角色自动走过去
- 御剑全程自动索敌，境界提升只增加飞剑数量；主动操作集中在踏云、剑阵与天劫三个功法
- 修为条常驻 HUD；进度过 78% 后头像亮起金环并呼吸提示，破境不再毫无预兆地弹出
- 斩杀、关底转相、破境与受击都会触发分档顿帧，精英以上追加慢放收束
- 击杀获得修为，境界提升时三选一强化，主动功法优先解锁
- 进化确认后免费显化一次对应能力，万剑 / 天雷 / 护体会立刻在战场兑现
- 每次进化触发限时“破境余势”：锋芒加快御剑、玄术加速功法、守元持续回盾
- 升级牌区分凡/灵/地/天品，可观星重掷或炼化回气；道藏记录把局内成长延续到战报
- 连斩剑势三档为 5 / 10 / 16，波次清空到下一波刷新之间冻结计时，换波不再必然断势
...
本地地址追加 `?qaVfxStress=1` 后进入任意章节，会跳过刷怪并以固定布局循环触发每轮 50 个命中、
死亡和飞剑轨迹样本。顶部诊断条实时显示活动/峰值特效、对象池命中率、丢弃量、回收量、长帧数
与平滑帧耗；该入口只对 `localhost` 和 `127.0.0.1` 生效。
