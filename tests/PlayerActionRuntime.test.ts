...
actions.enter('tribulation', 0.7);
assertEqual(actions.enter('autoAttack', 0.2), false, 'auto attack cannot interrupt a tribulation cast');
assertEqual(actions.enter('hit', 0.24), true, 'hit can interrupt a tribulation cast');
...
assertEqual(resolvePlayerAnimationFrame('tribulation', 0.35, 0.5).column, 2, 'actions follow state progress');
