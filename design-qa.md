...
## 2026-08-12 首页标题下方信息可读性优化

### 信息属性与问题

- `御剑破劫 · 三境问道` 是固定副标题。
- `道藏 N/6 · 真形 N/3 · 渡劫 N` 是动态进度，分别来自已发现路线数、已掌握修炼流派数和累计通关次数。
- P1：两行深褐文字直接覆盖在山门、屋檐与云雾细节上，背景明暗变化会吞掉笔画，动态进度尤其难读。

### 修正与证据

- 固定副标题调整为更深的墨褐色，并增加 4px 低透明宣纸色描边。
- 动态进度使用全透明度深墨色、18 号宋体和 3px 宣纸色描边，在浅云与深山区域都保持边缘分离。
- 修复后首页：`docs/design/audit-2026-08-12/16-header-progress-readable.png`。
- 用户问题截图 / 修复后同区域并排：`docs/design/audit-2026-08-12/17-header-progress-comparison.png`。
- 文字层级、居中位置与原设计保持不变，没有新增厚重色块遮挡山水背景。
